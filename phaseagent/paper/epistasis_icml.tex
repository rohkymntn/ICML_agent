...
most favorable regime; the functional assays are the opposite regime. We will
report, on GB1 binding and GFP fluorescence, the same zero-shot DPLM PLL
correlation against measured specific epistasis -- the floor that any learned
model must beat -- establishing that zero-shot PLM epistasis is a stability-coupled
signal that does not transfer to function. Those functional numbers are produced
by the GPU feature-extraction and scoring pipeline and fold in from their committed
artifacts under \texttt{outputs/epistasis/} as each scoring run completes.
