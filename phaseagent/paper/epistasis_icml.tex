...
model structurally cannot find. Fine-tuning DPLM end-to-end with LoRA across
proteins shows the skill transfers: on 25 held-out proteins that contribute no
training examples it reaches a median Spearman of $0.33$, above the $0.25$
zero-shot reference. All claims are backed by held-out evaluations on real
...
\begin{itemize}
  ...
  LoRA fine-tune of DPLM that transfers learned epistasis to 25 held-out proteins
  (median Spearman $0.33$, above the $0.25$ zero-shot reference), showing the
  skill is not memorized per assay.
\end{itemize}
...
clears $\spearman > 0.2$ for only 60\% of proteins (\cref{tab:zeroshot}). This is
...
$0.14$ on GFP (\cref{tab:model1}) -- a $+0.34$ and $+0.13$ gain over zero-shot.
...
during training, still clears the floor ($0.12$ on GB1, $0.08$ on GFP). Because
...
each reward. Ranking by Model 1's predicted specific epistasis lifts the mean
measured binding of the selection by $+1.09$ over the pool, recovering most of the
$+1.62$ oracle lift available to a perfect epistasis ranker
...
(\cref{fig:model2}). All numbers derive from the committed
\texttt{outputs/epistasis/model2\_oof\_GB1.csv}.

\begin{table}[t]
  \centering
  \caption{Model 2 design on held-out GB1 doubles ($n=12{,}000$): mean measured
  binding of the top $10\%$ selected from the additively-mediocre pool by each
  reward, and the lift over the additive pool mean. Ranking by Model 1's predicted
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{figures_epistasis/fig9_model2_design.pdf}
...
124 proteins and evaluated on the 25 held-out proteins that contribute no training
examples. With rank-64 adapters (12.9M trainable parameters) and 10 epochs, it
reaches a median per-held-out-protein Spearman of $0.33$ (mean $0.33$) between
predicted and measured specific epistasis, clearing $\spearman > 0.2$ for 80\% of
held-out proteins (\cref{tab:e2e}). This exceeds the zero-shot DPLM PLL reference
of $0.25$ on the same residual target (\cref{sec:results-zs}) \emph{on proteins
the head never saw}: the epistasis skill learned across proteins transfers to new
ones, rather than being memorized per assay. All numbers are read from the
committed \texttt{outputs/epistasis/e2e\_results.json}.
...
\begin{table}[t]
  ...
  Reference is the zero-shot DPLM PLL median per-protein Spearman on the same
  specific-epistasis residual. Source: \texttt{outputs/epistasis/e2e\_results.json}.}
  \label{tab:e2e}
  \begin{tabular}{lc}
    \toprule
