...
zero-shot epistasis signal is weak even on stability and, as we measure directly,
near zero on functional assays (\cref{sec:results-zs}). This motivates a \emph{trained}
...
\paragraph{Coevolution and direct-coupling models.}
A long line infers pairwise residue couplings from evolutionary sequence
covariation: Potts / direct-coupling models recover structural contacts
\citep{morcos2011direct} and predict mutation effects in an epistasis-aware,
unsupervised way \citep{hopf2017mutation}, and combinatorial deep mutational
scans confirm that specific, often sparse, higher-order epistasis genuinely
shapes function \citep{poelwijk2019learning}. These couplings are estimated from
cross-species alignment covariation and capture an average, evolutionarily and
structurally constrained interaction. Our target is different: the
assay-specific functional epistasis residual of a \emph{single} protein's
measured combinations after the additive and global layers are removed
(\cref{sec:decomp}), a quantity that family covariation does not directly
isolate. We therefore treat coevolutionary couplings as related context rather
than a matched baseline, and read specific epistasis out of one protein's
learned representation instead.
...
representations $H_i, H_j$ at the two mutated positions, project them, and
concatenate the two projections $\text{proj}(H_i),\text{proj}(H_j)$, their
bilinear product $\text{proj}(H_i) \odot \text{proj}(H_j)$, amino-acid identity
embeddings of the two substitutions, and the zero-shot
...
additive model is uninformative. We form this pool from the GB1 double mutants
whose additive$+$global prediction falls below the median -- the additively-mediocre half,
where the additive model rates every candidate roughly equal and cannot tell a
synergistic pair from an antagonistic one; only specific epistasis distinguishes
them. Model 2 ranks these candidates by predicted specific epistasis (from Model
1) and selects the top fraction.
...
baseline). (ii) The \emph{matched-additive control}: within narrow bins of the
additive$+$global prediction (so the additive value is held fixed and the global
saturation removed), the rank correlation
...
Code: \texttt{scripts/model2\_design.py}. Because every candidate GB1 double
mutant is measured in the complete pairwise assay
\citep{olson2014comprehensive}, this is a closed evaluation: no selected design
is unscored (Model 2 ranks measured candidates rather than generating new ones).
...
trained on the double mutants of 124 domains and evaluated on the 25
...
reranking} of randomly drawn candidates and an additive-ranking control. For design we additionally benchmark one strong generative comparator: a
...
($d=1280$) to $128$ dimensions through a GELU, then concatenates the two
projections $\text{proj}(H_i),\text{proj}(H_j)$, their bilinear product
$\text{proj}(H_i)\odot\text{proj}(H_j)$, $16$-dimensional embeddings of the four
amino-acid identities (wild-type and mutant at both sites), and the $z$-scored
zero-shot PLL feature; a LayerNorm followed by a
...
$z$-scored per dimension, and the head never receives position indices. Every
randomized procedure -- the cross-validation splits, the head and LoRA weight
initialization, the end-to-end fine-tune's per-protein subsampling, and the
$2{,}000$-resample bootstraps used for all confidence intervals -- runs with a
fixed seed ($0$), so the reported numbers and committed artifacts reproduce
deterministically from the committed scripts. Full configuration:
\texttt{scripts/train\_model1.py}.
...
\texttt{model1\_*\_summary.json:zeroshot\_spearman} keys, and they
...
than memorized from position identity. That a held-out predictor recovers this
residual at all -- with bootstrap intervals excluding zero -- is itself evidence
that functional specific epistasis is real signal and not measurement scatter,
since noise cannot be predicted out of fold. Both rows are derived from the committed
...
\begin{table}[t]
  ...
  \begin{tabular}{lccc}
    ...
    \cmidrule(lr){3-4}
    Assay & Zero-shot & Doubles & Position \\
    \midrule
...
additively-mediocre candidates (those below the median additive$+$global prediction, where
...
recovering most of the $+1.61$ oracle lift available to a perfect epistasis ranker
...
the loop: within deciles of the additive$+$global prediction, where additive value is held
...
\begin{table}[t]
  ...
  \begin{tabular}{lcc}
    \toprule
    Selection rule & Top-10\% & Lift \\
    \midrule
    Additive (pool mean) & $-3.62$ & $0.00$ \\
    Best-of-$N$ (zero-shot DPLM PLL) & $-3.84$ & $-0.22$ \\
    Model 1 (predicted epistasis) & $-2.53$ & $+1.09$ \\
    Oracle (true epistasis) & $-2.00$ & $+1.61$ \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{figure}[t]
  ...
  across additive$+$global prediction deciles. Regenerates from
...
head-to-head competitor. After $3$ epochs of reward-weighted fine-tuning, the generated
library reaches mean function $0.16$ -- above a random library ($0.08$) and the assay-wide
mean ($0.08$), but well below the unconditioned DPLM library ($1.09$) and far short of the
top-library ceiling ($2.12$); coverage of measured combinations is $0.974$. On this closed
assay the reward-guided generator does not clear even the unconditioned-DPLM library: the
wild-type GB1 interface is already high-function, so an unconditioned protein LM samples
strong combinations on its own. This reinforces our central finding -- guidance from a
sequence LM, like the zero-shot reranker, does not by itself capture the specific epistasis
a representation-trained predictor exposes. All numbers derive from the committed
...
contribute no training examples. With rank-64 adapters (12.85M trainable
...
Model 1's held-out-position transfer is the hardest regime and is correspondingly
the weakest we report, though it stays above chance (\cref{sec:results-model12}).
...
our own. Finally, all results are
...
This work studies how to predict and design epistatic interactions in protein
function. The methodological aim is to advance machine learning for biological
sequence modeling, and the most direct beneficial applications are in protein
engineering -- for example designing enzymes, binders, or stabilized therapeutics
where combinations of mutations must act synergistically. As with any protein-design
method, the same capability could in principle be applied to engineer harmful
proteins; we note that our experiments are entirely \emph{in silico}, use only
well-characterized and benign public assays (GB1 IgG-binding, GFP fluorescence, and
domain folding stability), and include no wet-lab synthesis or validation, so the
immediate dual-use risk of the specific results reported here is low. We also
emphasize that the predictive correlations we report, while above the relevant
baselines, are far from accurate enough to substitute for experimental measurement,
which limits the potential for misuse from the models alone. We encourage the use of
established biosecurity screening for any downstream sequence design built on methods
of this kind.
