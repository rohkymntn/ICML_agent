...
model structurally cannot find. We further fine-tune DPLM end-to-end with LoRA
across proteins and evaluate cross-protein generalization on held-out proteins.
All claims are backed by held-out evaluations on real deep-mutational-scanning
data.
\end{abstract}
...
\begin{itemize}
  ...
  LoRA fine-tune of DPLM evaluated on held-out proteins, testing whether learned
  epistasis transfers beyond the training proteins.
\end{itemize}
...
pseudo-likelihood tracks the \emph{global}, stability-coupled component well, but
its zero-shot epistasis signal on functional assays is near zero
(\cref{sec:results-decomp}). This motivates a \emph{trained} read-out rather than
a zero-shot score.
...
\end{equation}
as a residual base. A small head regresses $\eps_{ij}$. Crucially, the head never
...
function of the selected variants versus the pool mean and versus the oracle
top-selection. (ii) The \emph{matched-additive control}: within narrow bins of
...
pairwise epistasis head, trained on the double mutants of $\sim$120 proteins and
evaluated on $\sim$25 \emph{held-out proteins} that contribute no training
examples. The headline metric is the median per-held-out-protein Spearman
...
generative comparators (discrete-diffusion guidance and reward-guided refinement)
are required only if a generative design claim is made; our Model 2 is a ranking
policy over measurable candidates, so the additive and best-of-$N$ controls are
...
$0.42$ kcal/mol, roughly four times the $\sim 0.1$ kcal/mol measurement noise of
the assay, and it exceeds $0.3$ kcal/mol for 85\% of proteins. Specific epistasis
accounts for at least 10\% of double-mutant variance in 98\% of proteins (median
share 28\%). Specific epistasis is therefore both large and ubiquitous: a real,
...
\begin{figure}[t]
  ...
  The median ($0.42$ kcal/mol, red) is far above the assay measurement-noise band
  (grey): the signal a learned model must predict.}
  \label{fig:specific}
\end{figure}
...
naturalness is coupled to stability. The functional assays are the opposite
regime. We will report, on GB1 binding and GFP fluorescence, the rank correlation
between zero-shot DPLM PLL epistasis (\cref{eq:zs}) and measured specific
epistasis -- the floor that any learned model must beat -- and contrast it with
the much higher stability correlation, establishing that zero-shot PLM epistasis
is a stability signal that does not transfer to function. These functional
numbers are produced by the GPU feature-extraction and scoring pipeline and are
folded in from their committed artifacts under \texttt{outputs/epistasis/} as each
run completes; the present draft reports the committed stability decomposition in
full (\cref{tab:decomp}).

\subsection{Model 1 and Model 2}
\label{sec:results-model12}

Model 1 (\cref{sec:model1}) is evaluated on held-out doubles and held-out
positions for each functional assay, against the additive and zero-shot baselines
(\cref{sec:exp}); Model 2 (\cref{sec:model2}) is evaluated by gain-of-function
recovery and the matched-additive within-bin Spearman. Each number is traced to a
committed CSV/parquet under \texttt{outputs/epistasis/} in \texttt{CLAIMS.md}; the
quantitative tables are added as their run artifacts are committed.

\subsection{Cross-protein generalization}
\label{sec:results-e2e}

The end-to-end LoRA fine-tune (\cref{sec:e2e}) is evaluated on the held-out
proteins; the headline median per-held-out-protein Spearman is read from the
committed \texttt{outputs/epistasis/e2e\_results.json} and reported here once the
...
is measurable, and may not represent all functional landscapes. The global layer
is a per-protein monotonic link fit on the same assay it explains; we therefore
report specific epistasis only as a held-out residual, never as in-sample fit.
...
against discrete-diffusion guidance or reward-guided refinement; Model 2 is a
