...
Spearman of $0.33$, above the $0.25$ zero-shot reference. All claims are backed
...
\begin{itemize}
  ...
  Megascale stability domains (median Spearman $0.33$, above the $0.25$ zero-shot
  reference), showing the skill is not memorized per protein. This cross-protein
...
zero-shot DPLM PLL reference of $0.25$ on the same residual target
(\cref{sec:results-zs}) \emph{on proteins the head never saw}: the epistasis skill
learned across proteins transfers to new ones, rather than being memorized per
protein. The transfer is demonstrated on stability because that is the only corpus
with enough proteins for a held-out-protein split; transferring the
\emph{functional} skill across proteins is the analogous open problem
(\cref{sec:limitations}). All numbers are read from the committed
\texttt{outputs/epistasis/e2e\_results.json}.
